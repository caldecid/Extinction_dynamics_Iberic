...
##filtering data
data <- peninsula_genus[peninsula_genus$Genus %in% peninsula_genera_phy$tip.label, ]

# reorder data
data <- data[match(peninsula_genera_phy$tip.label, data$Genus), ]
...
K_result <- phylosig(peninsula_genera_phy, threatened_vector,
                     nsim = 100, test = TRUE, method = "K")

lambda_result <- phylosig(peninsula_genera_phy,
...
##calculating Bloomberg's K and Pagel's lambda
##filtering data
data <- andalucia_genus[andalucia_genus$Genus %in% andalucia_genera_phy$tip.label, ]

# reorder data
data <- data[match(andalucia_genera_phy$tip.label, data$Genus), ]
...
K_result_and <- phylosig(andalucia_genera_phy, threatened_vector,
                         nsim = 100, test = TRUE, method="K")

lambda_result_and <- phylosig(andalucia_genera_phy,
                          threatened_vector, nsim = 100,
                          test = TRUE, method="lambda")
