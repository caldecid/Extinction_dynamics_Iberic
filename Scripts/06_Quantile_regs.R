...
# Peninsula 
pen_data <- read_csv("Data/Processed/peninsula_ages_total.csv")
non_data <- pen_data %>%  filter(ext_fraction == "low_ex") %>%  
  mutate(mean_age = age, 
         ext_fraction = "Not Corrected")
pen_data <- non_data %>%  bind_rows(pen_data)

# Andalucia
and_data <- read_csv("Data/Processed/andalucia_ages_total.csv")
non_data <- and_data %>%  filter(ext_fraction == "low_ex") %>%  
  mutate(mean_age = age, 
         ext_fraction = "Not Corrected")
and_data <- non_data %>%  bind_rows(and_data)


# FUnction
quant_analysis <- function(x, name_file = "pen") {
...
lab_lev <- c("Not corrected",
             ...
             "High extinction")
data_quant_pen <- x %>%
  mutate(Corrected = factor(ext_fraction,
...
for (i in 1:n) {
  ...
  rq_pen_temp <- quantreg::rq(proportion_threatened ~ mean_age, data = data_quant_pen_temp,
                              tau = q10)
...
png(paste0("Figures/Figure_", name_file,"_quant.png"), 
    width = 30, height = 20,
...
quantile_pen <- data_quant_pen %>% 
  ...
  scale_color_gradientn(
    colors = colors_scale  # Light green to dark blue
  ) +
...
res_and <- quant_analysis(and_data, "and")
res_pen <- quant_analysis(pen_data, "pen")
